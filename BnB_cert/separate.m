...
function [p0, p1, zeroOK, oneOK] = separate(prob, branchvar,options)
if (length(prob.vartype) >= 1)
    n    = size(prob.H,1);
    
    this  = prob.vartype(branchvar);
    others= [1:this-1, this+1:n];
...
%         p1.Gx= prob.Gx;
    ...
    if isfield(prob,'iter')
        ...
        if isfield(prob,'AS_stack')
             p0.AS_stack = prob.AS_stack;
             p1.AS_stack = prob.AS_stack;
        end
...
%     	p1.b_th= []; 
   ...
   if isfield(prob, 'Q_th')
    ...
    if options.warm_start
        AS_parent = []; AS_par = [];
        if isfield(prob, 'AS_parent') 
            AS_parent = prob.AS_parent;
        elseif isfield(prob, 'AS') 
            AS_parent = prob.AS;
        end
        
        m = length(p0.b); %number of constraints
        nb = length(prob.vartype); %%number of binary variables
                        
        options.keep_AS_order = false;
        %___________________________
        % Approach 1: order does not remain
        if ~options.keep_AS_order
