function  S_stack_out = push_proc(childs,T_stack,Theta, new_S,S_stack,opts)  %[T_stack_out, S_stack_out]   
  
  epsi = opts.epsi; % 1e-4; % tolerance for zero
  stack_size = length(T_stack);
  S_stack_out = cell(0);
  Nr_childs = length(childs);
  max_act_nod = opts.bound_active_nodes;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if Nr_childs == 0
      % there is no child nodes--> cut here!
          ...
          switch opts.method
                case 'depth'
                    ...
                    % put in first always (no need to compare)
                      T_stack_out = {childs{:}, T_stack{:}}; 
                      if (opts.subopt_Mcut && (length(T_stack_out) > max_act_nod)) %apply M-cut suboptimality
                          ind_start = length(T_stack_out)- max_act_nod + 1;
                          sortedT = {T_stack_out{ind_start:end}}; 
                          T_stack_out = sortedT;
                      end                                                    
                    % update new tuple to put in S
                      new_S.Theta = Theta;
                ...
                case 'breadth'
                    ...
                    if isempty(jj)
                     ...
                     if kk >= stack_size  
                        T_stack_out = { T_stack{:}, childs{:}}; 
                     else
                        T_stack_out = { T_stack{1:kk}, childs{:}, T_stack{kk+1:end}};
                     end  
                     if (opts.subopt_Mcut && (length(T_stack_out) > max_act_nod)) %apply M-cut suboptimality
                          ind_start = length(T_stack_out)- max_act_nod + 1;
                          sortedT = {T_stack_out{ind_start:end}}; 
                            ...
                            if (norm(B_diff{k}) <= epsi) && (abs(C_diff{k}) <= epsi) % norm(A_diff{k}) <= epsi %for QP case
                                ...
                                if flag_devide(k)    
                                  % partition region to_add to two new regions 
                                   ...
                                   p0.Theta = Polyhedron; %  %empty the polyhedron
                                   p0.Theta = Theta2; % update polyhedron
                                   %p1.Theta = Polyhedron; p1.Theta = Theta2;
                                   childs{1}.Theta = Theta2; % for p0
                                   childs{end}.Theta = Theta2; %for p1
                                   ...
                                   kk = k-1;
                                     ...
                                     if k == stack_size % ( && flag_reg_left)
                                         %keyboard;
                                         childs{1}.Theta = Theta; % for p0
                                         childs{end}.Theta = Theta; %for p1
  ...
  end %if there is no child nodes
